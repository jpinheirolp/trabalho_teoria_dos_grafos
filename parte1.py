        ...
        while(contador_explorados > 0):
            ...
            for vertice_vizinho in self.gera_vertices_adjacentes(vertice_atual):
                if vertices_explorados[vertice_vizinho]:
                    continue
                peso_aresta = self.retorna_peso_aresta(
                ...
                if custo_vizinho > peso_aresta:
                    ...
                    vetor_arestas_leves[vertice_vizinho] = [
                        vertice_atual, vertice_vizinho, peso_aresta]
                    fila_prioridade.put(
                        (vetor_custos[vertice_vizinho], vertice_vizinho))
    ...
    def executa_dijkstra(self, vertice_origem, vertices_destino=None):
        ...
        while((contador_explorados > 0) and (contador_destinos < len(vertices_destino))):
            ...
            for vertice_vizinho in self.gera_vertices_adjacentes(vertice_atual):
                ...
                if vetor_distancias[vertice_vizinho] > (vetor_distancias[vertice_atual] + peso_aresta):
                    ...
                    fila_prioridade.put(
                        (vetor_distancias[vertice_vizinho], vertice_vizinho))
        for vertice_destino in vertices_destino:
            ...
            while (vertice_pai != vertice_origem):
                ...
                menorcaminho.append([vertice_pai, vertice_filho, self.retorna_peso_aresta(
                    vertice_pai, vertice_filho)])
                vertice_filho = vertice_pai
            resultado.append(
                [vertice_destino, vetor_distancias[vertice_destino], menorcaminho])
    ...
    def executa_bellman_ford(self, vertice_destino, vertices_origem=None):
        ...
        while tamanho_menor_caminho < self.numero_vertices:
            ...
            for vertice_atual in range(self.numero_vertices):
                ...
                for vertice_vizinho in self.gera_vertices_adjacentes(vertice_atual):
                    nova_distancia = self.retorna_peso_aresta(
                        vertice_atual, vertice_vizinho) + vetor_distancias[vertice_vizinho]
        ...
        for vertice_origem in vertices_origem:
            ...
            while (vertice_pai != vertice_destino):
                ...
                vertice_filho = vertice_pai
            resultado.append(
                [vertice_origem, vetor_distancias[vertice_origem], menorcaminho])
...
class grafo_sem_pesos():
    ...
    def busca_largura(self, vertice_raiz, funcao_auxiliar=lambda x, y: 0, condicao_parada=False):
        vetor_explorados = np.zeros(
            self.numero_vertices)  # vetor nivel do vertice
        ...
        while (not fila.empty()) and ((retorno_func_auxiliar == None) or (not condicao_parada)):
            ...
            for vertice_adjacente in self.gera_vertices_adjacentes(vertice_sendo_explorado):
                if vetor_explorados[vertice_adjacente] == 0:
                    retorno_func_auxiliar = funcao_auxiliar(
                        vertice_adjacente, vertice_sendo_explorado)
    ...
    def calcula_diametro_grafo_estimado(self, numero_distancias):
        ...
        while contador < numero_distancias:
            distancia = self.calcula_distancia_vertices(random.randint(
                0, self.numero_vertices-1), random.randint(0, self.numero_vertices-1))
            if distancia == None:
                distancia = 0
    ...
    def calcula_diametro_grafo(self):
        ...
        for vertice_raiz in range(self.numero_vertices):
            ...
            def funcao_auxiliar(vertice_filho, vertice_pai):
                ...
                vetor_nivel_arvore[vertice_filho] = vetor_nivel_arvore[vertice_pai] + 1
                diametro[0] = max(
                    vetor_nivel_arvore[vertice_filho], diametro[0])
                return None
    ...
    def descobre_componentes_conexas(self):
        ...
        # o '-1' e so pra corrigir o problema do ultimo for n rodar por a lista estar vazia
        lista_componentes = [[-1]]
        componente_conexa = [0]
        ...
        for vertice_raiz in range(self.numero_vertices):
            if vertices_conhecidos[vertice_raiz] == 0:
                self.busca_largura(vertice_raiz, funcao_auxiliar)
                # acrescentando e contabilizando a raiz que faltava
                componente_conexa[0] += 1
        ...
        # remove 0 '[-1]' pois ele sempre sera o ultimo elemento
        lista_componentes.pop()
...
class grafo_lista_adjacencia_peso(grafo_com_pesos):
    def __init__(self, arestas, numero_vertices):
        ...
        for lista in range(self.numero_vertices):
...
class grafo_lista_adjacencia_peso_direcionado(grafo_com_pesos):
    def __init__(self, arestas, numero_vertices):
        self.numero_vertices = numero_vertices
        self.numero_arestas = len(arestas)
        self.lista_adjacencias_sai = []
        self.lista_adjacencias_entra = []
        for lista in range(self.numero_vertices):
            self.lista_adjacencias_sai.append([])
        for lista in range(self.numero_vertices):
            self.lista_adjacencias_entra.append([])
        self.lista_graus = np.zeros(numero_vertices)
        for aresta in arestas:
            if aresta[0] == aresta[1]:
                print("aresta não pode ir de um vertice para ele mesmo")
            if (aresta[1] in self.lista_adjacencias_sai[aresta[0]] or aresta[0] in self.lista_adjacencias_entra[aresta[1]]):
                print("Aresta "+str(aresta)+"já existe no grafo")
                continue

            self.lista_adjacencias_sai[aresta[0]].append([aresta[1], aresta[2]])

            self.lista_adjacencias_entra[aresta[1]].append([aresta[0], aresta[2]])

            # bisect.insort(self.lista_adjacencias[aresta[1]],aresta[0])
            # bisect.insort(self.lista_adjacencias[aresta[0]],aresta[1])

    def gera_vertices_adjacentes(self, vertice_origem, reverter=False):
        gerador = self.lista_adjacencias_sai[vertice_origem]
        if reverter:
            gerador = gerador[::-1]
        for vertice in gerador:
            yield vertice[0]

    def gera_vertices_entram(self, vertice_origem, reverter=False):
        gerador = self.lista_adjacencias_entra[vertice_origem]
        if reverter:
            gerador = gerador[::-1]
        for vertice in gerador:
            yield vertice[0]
    ...
    def retorna_peso_aresta(self, vertice1, vertice2):
        gerador = self.lista_adjacencias_entra[vertice1]
        for vertice in gerador:
            if vertice[0] == vertice2:
                return vertice[1]

        gerador = self.lista_adjacencias_sai[vertice1]
        for vertice in gerador:
            if vertice[0] == vertice2:
                return vertice[1]
...
parser.add_argument(
    "-k", "--kind", help="Representacao das arestas em  matriz de adjacencia(1) ou lista de adjacencia(2) ")
...
def processarArquivoSaida(grafo, arquivosaida):
    ...
    # BFS
    f.write("Pai do vertice 10 na busca BFS em 1:" +
            str(arvore_busca1BFS[0][9])+"\n")
    f.write("Pai do vertice 20 na busca BFS em 1:" +
            str(arvore_busca1BFS[0][19])+"\n")
    f.write("Pai do vertice 30 na busca BFS em 1:" +
            str(arvore_busca1BFS[0][29])+"\n")
    f.write("Pai do vertice 10 na busca BFS em 2:" +
            str(arvore_busca2BFS[0][9])+"\n")
    f.write("Pai do vertice 20 na busca BFS em 2:" +
            str(arvore_busca2BFS[0][19])+"\n")
    f.write("Pai do vertice 30 na busca BFS em 2:" +
            str(arvore_busca2BFS[0][29])+"\n")
    f.write("Pai do vertice 10 na busca BFS em 3:" +
            str(arvore_busca3BFS[0][9])+"\n")
    f.write("Pai do vertice 20 na busca BFS em 3:" +
            str(arvore_busca3BFS[0][19])+"\n")
    f.write("Pai do vertice 30 na busca BFS em 3:" +
            str(arvore_busca3BFS[0][29])+"\n")
    f.write("Pai do vertice 10 na busca DFS em 1:" +
            str(arvore_busca1DFS[0][9])+"\n")
    f.write("Pai do vertice 20 na busca DFS em 1:" +
            str(arvore_busca1DFS[0][19])+"\n")
    f.write("Pai do vertice 30 na busca DFS em 1:" +
            str(arvore_busca1DFS[0][29])+"\n")
    f.write("Pai do vertice 10 na busca DFS em 2:" +
            str(arvore_busca2DFS[0][9])+"\n")
    f.write("Pai do vertice 20 na busca DFS em 2:" +
            str(arvore_busca2DFS[0][19])+"\n")
    f.write("Pai do vertice 30 na busca DFS em 2:" +
            str(arvore_busca2DFS[0][29])+"\n")
    f.write("Pai do vertice 10 na busca DFS em 3:" +
            str(arvore_busca3DFS[0][9])+"\n")
    f.write("Pai do vertice 20 na busca DFS em 3:" +
            str(arvore_busca3DFS[0][19])+"\n")
    f.write("Pai do vertice 30 na busca DFS em 3:" +
            str(arvore_busca3DFS[0][29])+"\n")
    # 5
    f.write("Distancia vertice 10 20:" +
            str(grafo.calcula_distancia_vertices(10, 20))+"\n")
    f.write("Distancia vertice 10 30:" +
            str(grafo.calcula_distancia_vertices(10, 30))+"\n")
    f.write("Distancia vertice 20 20:" +
            str(grafo.calcula_distancia_vertices(20, 30))+"\n")
    # 6
    ...
    f.write("Tamanho da Maior Componente Conexa:" +
            str(componentes_conexas[0][0])+"\n")
    f.write("Tamanho da Menor Componente Conexa:" +
            str(componentes_conexas[-1][0])+"\n")
    f.close()
# Debug
...
if len(arg1[0]) == 3 and temnegativo == 0:
    ...
    print("{};{};{};{};{};{};{};{}".format(cleanfilename, (somatempo/100), distancia1_10,
          distancia1_20, distancia1_30, distancia1_40, distancia1_50, pesomst))
...
if len(arg1[0]) == 3 and temnegativo == 1:
    ...
    grafo = grafo_lista_adjacencia_peso_direcionado(arg1, arg2)
    start = timer()
    for i in range(100):
